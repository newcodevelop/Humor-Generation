...
distilbert = DistilBertForSequenceClassification.from_pretrained('./results')
...
class Encoder(nn.Module):
    ...
    def forward(self, src, src_mask, is_discriminator=False, if_req=False):
        ...
        if if_req:
          print('in if_req')
          print(src.shape)
          print(self.tok_embedding.weight.shape)
          src_ = self.dropout( (torch.mm(src,self.tok_embedding.weight)*self.scale) + self.pos_embedding(pos) )
        else:
...
model = Seq2Seq(enc, dec_h,dec_n, SRC_PAD_IDX, TRG_PAD_IDX, device).to(device)
...
optimizer_nh = torch.optim.Adam(disc_nh.parameters(), lr = LEARNING_RATE)
optimizer_h = torch.optim.Adam(disc_h.parameters(), lr = LEARNING_RATE)
...
CCC = []
TURN=0
def reinforce(list_of_src_tokens,list_of_trg_tokens, src_field, trg_field, model, device,lab_, max_len = 50):
...
def train(model, iterator, optimizer, criterion, clip, epoch_nos):
    ...
    for i_, batch in tqdm(enumerate(iterator)):
        ...
        l1_s = int(loss_ce1.size()[0]/16)
        l2_s = int(loss_ce2.size()[0]/16)

        loss_ce1 = loss_ce1.reshape((16,l1_s))
        loss_ce2 = loss_ce2.reshape((16,l2_s))
        l1 = lab.repeat_interleave(l1_s).reshape(16,-1)
        l2 = lab.repeat_interleave(l2_s).reshape(16,-1)
        ...
        if epoch_nos>0:
            ...
            if i_%50==0:
              print('********inside with i {}***************'.format(i_))
              eval(model, test_iterator, criterion)


            print(loss_ce1,loss_ce2,loss_mse,loss_st,rl_loss,mi)
        else:
...
for epoch in range(N_EPOCHS):
    ...
    # if valid_loss < best_valid_loss:
    #     best_valid_loss = valid_loss
    torch.save(model.state_dict(), './double_decoder/tr_model_full_reward_epoch{}.pt'.format(epoch))
